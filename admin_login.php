...
} elseif ($_SESSION['admin_login_attempts'] >= 5 && (time() - $_SESSION['admin_last_attempt']) < 300) {
        ...
        if (empty($email) || empty($password)) {
            ...
            try {
                ...
                if ($user && password_verify($password, $user['password']) && $user['is_admin'] == 1) {
                    ...
                    $_SESSION['admin_login_attempts'] = 0; // Reset attempts on success
                    
                    session_regenerate_id(true);
                    $_SESSION['admin_csrf_token'] = bin2hex(random_bytes(32));
                    
                    header("Location: admin_dashboard.php?success=" . urlencode("Welcome back, {$user['name']}"));
                    exit;
                } else {
...
<head>
    ...
    <title>Admin Login - Maa Travels</title>
    ...
    <main class="container mx-auto py-10 px-4">
        ...
        <div class="max-w-md mx-auto bg-white shadow-lg p-6 rounded-lg">
            ...
            <div class="text-center mt-4">
                <p class="text-gray-600">Regular user? <a href="login.php" class="text-blue-500 hover:underline">Login here</a></p>
            </div>
        </div>
    </main>
    <script>
