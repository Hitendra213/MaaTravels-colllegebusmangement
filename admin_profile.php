    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['admin_csrf_token']) {
        ...
        if (empty($errors)) {
            try {
                ...
                $query = "UPDATE users SET name = ?, email = ?, mobile = ?";
                ...
                $_SESSION['admin_csrf_token'] = bin2hex(random_bytes(32));
                header("Location: admin_profile.php?success=" . urlencode($successMessage));
                exit;
            } catch (PDOException $e) {
...
<head>
    ...
    <title>Admin Profile - Maa Travels</title>
...
<body class="bg-gray-100">
    ...
    <main class="container mx-auto py-16 px-4">
        <h1 class="text-3xl font-semibold text-center mb-6 text-gray-800">Admin Profile</h1>
        <div class="max-w-lg mx-auto bg-white shadow-lg p-6 rounded-lg">
            ...
            <form action="admin_profile.php" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="name" name="name" value="<?php echo htmlspecialchars($admin['name'], ENT_QUOTES, 'UTF-8'); ?>" 
                           class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" required>
                </div>
                <div class="mb-4">
                    <label for="email" class="block text-sm font-medium text-gray-700 mb-2">Email</label>
                    <input type="email" id="email" name="email" value="<?php echo htmlspecialchars($admin['email'], ENT_QUOTES, 'UTF-8'); ?>" 
                           class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" required>
                </div>
                <div class="mb-4">
                    <label for="mobile" class="block text-sm font-medium text-gray-700 mb-2">Mobile</label>
                    <input type="text" id="mobile" name="mobile" value="<?php echo htmlspecialchars($admin['mobile'], ENT_QUOTES, 'UTF-8'); ?>" 
                           class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" required>
                </div>
                <div class="mb-4">
